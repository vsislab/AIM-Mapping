...
from utils.gnn import AsyMinusGNN
...
class RL_Policy(nn.Module):
    ...
    def __init__(self, obs_shape, action_space, model_type='gconv',
        ...
        if model_type == 'ans':
            ...
            self.network = AsyMinusGNN(obs_shape, base_kwargs.get('num_gnn_layer') * ['self', 'cross'], base_kwargs.get('use_history'), base_kwargs.get('ablation'))
        else:
        ...
        self.critic_optimiz_mi = optim.Adam(filter(lambda p: p.requires_grad,
            self.network.MI_eva.parameters()), lr=lr[0] * lr[1], eps=eps)
    ...
    def forward(self, inputs, rnn_hxs, masks, extras, predict_unexplored=False):
        if extras is None:
            ...
            return self.network(inputs, rnn_hxs, masks, extras, predict_unexplored)

    def act(self, inputs, rnn_hxs, masks, extras=None, deterministic=False):
        
        value,loss, actor_features, rnn_hxs = self(inputs, rnn_hxs, masks, extras)
        ...
        return value,loss, action, action_log_probs, rnn_hxs, actor_features

    def get_value(self, inputs, rnn_hxs, masks, extras=None):
        value, mi_loss,actor_features, _ = self(inputs, rnn_hxs, masks, extras)
        return value, actor_features

    def evaluate_actions(self, inputs, rnn_hxs, masks, action, extras=None):
        # do the predict of unexplored area
        value,mi_loss, actor_features, rnn_hxs, unexplored_value = self(inputs, rnn_hxs, masks, extras, True)
        dist = self.dist(actor_features)

        action_log_probs = dist.log_probs(action)
        dist_entropy = dist.entropy().mean()

        return value,mi_loss,action_log_probs, dist_entropy, rnn_hxs, actor_features, unexplored_value
    ...
    def load(self, path, device):
        ...
        self.critic_optimiz_mi = optim.Adam(filter(lambda p: p.requires_grad,
            self.network.MI_eva.parameters()), lr=1e-3)
        # state_dict = torch.load(path, map_location=lambda storage, loc: storage)
        state_dict = torch.load(path, map_location=device)
        self.network.load_state_dict(state_dict['network'])
        self.actor_optimizer.load_state_dict(state_dict['actor_optimizer'])
        self.critic_optimizer.load_state_dict(state_dict['critic_optimizer'])
        self.critic_optimiz_mi.load_state_dict(state_dict['critic_optimiz_mi'])
        del state_dict
    ...
    def save(self, path):
        state = {
            ...
            'critic_optimiz_mi': self.critic_optimiz_mi.state_dict(),
        }
