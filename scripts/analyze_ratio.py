...
def main():
    ...
    parser.add_argument('--dir', type=str, default='infonce915_mi1_1')
    parser.add_argument('--dataset', type=str, default='mp3dhq0')
    parser.add_argument('--subset', type=str, default='acdefghijkl')
    ...
    for lst, r in zip(mean_len, split):
        ...
        num_tasks = len(lst)
        completed_tasks = sum([1 for v in lst if v <= 2500 ]) 
        completion_rate = 100 * completed_tasks / num_tasks

        print('任务完成率 [{}~{}m2]: {:.1f}% (共 {} 个任务，完成 {} 个任务)'.format(
            r[0], r[1], completion_rate, num_tasks, completed_tasks
        ))
