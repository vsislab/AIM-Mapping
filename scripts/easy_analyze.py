...
def main():
    ...
    parser.add_argument('--dir', type=str, default='periodic_29000004')
    parser.add_argument('--dataset', type=str, default='mp3dhq0')
    parser.add_argument('--subset', type=str, default='abcdefghijkl')
    args = parser.parse_args()

    result = []
    split = [[0, 60], [60, 100], [100, 1000]]
    mean_len = [[], [], []]
    mean_len2 = [[], [], []]
    ...
    for lst, r in zip(mean_len, split):
        if lst == []:
            continue
        print('mean length [{}~{}m2]: {:.2f}'.format(r[0], r[1], sum(lst) / len(lst)))
    for lst2, r in zip(mean_len2, split):
        if lst2 == []:
            continue
        print('mean exploration rate [{}~{}m2]: {:.1f}%'.format(r[0], r[1], 100 * sum(lst2) / len(lst2)))
    for lst,lst2, r in zip(mean_len,mean_len2, split):
        if lst == []:
            continue
        ...
        for x in range(num_tasks):
            time=lst[x]
            wcl=lst2[x]
            if time <= 2500 and wcl >0.8:
                completed += 1
        completion_rate = 100 * completed / num_tasks

        print('任务完成率 [{}~{}m2]: {:.1f}% (共 {} 个任务，完成 {} 个任务)'.format(
            r[0], r[1], completion_rate, num_tasks, completed
        ))
