...
class PPO():
    ...
    def __init__(
        ...
        self.critic_optimiz_mi=actor_critic.critic_optimiz_mi
        ...
        self.predict_loss_coef = predict_loss_coef
    ...
    def update(self, rollouts):
        ...
        for e in range(self.ppo_epoch):
            ...
            for sample in data_generator:
                ...
                values, mi_loss,action_log_probs, dist_entropy, _, action_feature, unexplored_value = \
                    self.actor_critic.evaluate_actions(
                        sample['obs'], sample['rec_states'],
                        sample['masks'], sample['actions'],
                        extras=sample['extras']
                    )
                # predict loss about unexplored area
                batch_size = sample['obs'].size(0)
                explored_gt = torch.sum(sample['obs'][:,4,:,:].reshape(batch_size,-1),dim=-1)
                explorable_gt = torch.sum(sample['obs'][:,5,:,:].reshape(batch_size,-1),dim=-1)
                exploration_ratio = explored_gt/explorable_gt

                loss_fun_predict = torch.nn.MSELoss(reduction='mean')
                predict_loss = loss_fun_predict(exploration_ratio, unexplored_value)



                augmentation = sample['augmentation']
                value_preds = values if augmentation else sample['value_preds']
                mi_loss_preds = mi_loss if augmentation else sample['mi_loss_preds']
                ...
                if self.use_clipped_value_loss:
                    ...
                    mi_loss_preds_clipped = mi_loss_preds + \
                                        (mi_loss - mi_loss_preds).clamp(
                                            -self.clip_param, self.clip_param)
                    mi_losses = (mi_loss - returns).pow(2)
                    mi_losses_clipped = (mi_loss_preds_clipped
                                            - returns).pow(2)
                    mi_loss_loss = .5 * torch.max(mi_losses,
                                                mi_losses_clipped).mean()
                    mi_clip = (torch.abs(mi_loss - mi_loss_preds) <= self.clip_param).float().mean().item()
                ...
                print('a/v clip: {:.3f}/{:.3f}'.format(action_clip, value_clip,mi_clip))

                self.actor_optimizer.zero_grad()
                self.critic_optimizer.zero_grad()
                self.critic_optimiz_mi.zero_grad()
                (value_loss * self.value_loss_coef + mi_loss_loss + action_loss * self.action_loss_coef - dist_entropy * self.entropy_coef + self.predict_loss_coef * predict_loss).backward()
                ...
                self.critic_optimiz_mi.step()

                if not augmentation:
                    value_loss_epoch += value_loss.item()
                    mi_loss_epoch += mi_loss.item()
                    action_loss_epoch += action_loss.item()
                    dist_entropy_epoch += dist_entropy.item()
                    predict_loss_epoch += predict_loss.item()

        num_updates = self.ppo_epoch * self.num_mini_batch

        value_loss_epoch /= num_updates
        mi_loss_epoch /= num_updates
        action_loss_epoch /= num_updates
        dist_entropy_epoch /= num_updates
        predict_loss_epoch /= num_updates

        return value_loss_epoch, mi_loss_epoch,action_loss_epoch, dist_entropy_epoch, predict_loss_epoch
