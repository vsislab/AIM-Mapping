        ...
        if self.config['scene'] == 'empty':
            ...
            scene = StaticIndoorScene(
                ...
                pybullet_load_texture=self.config.get(
                    'pybullet_load_texture', False) and self.config['scene_id'] not in ['Cross'],
            )
            self.simulator.import_scene(
                scene, load_texture=self.config.get('load_texture', True) and self.config['scene_id'] not in ['Cross'])
    ...
    def reset(self):
        ...
        for i in range(max_reset):
            ...
            if self.task is not None:
                ...
                idx_agent = 0
                last_pos = None if self.config['scene_id'] not in ['Cross'] else np.array([0, 0])
