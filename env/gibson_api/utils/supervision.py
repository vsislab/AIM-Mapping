...
class GibsonMaps(object):
    def __init__(self, env, floor_num, N=int(1e6), resolution=5, padding=0):
        # convert to cm
        ...
        self.y = pts[:, 1]
        zx = pts[:, [2, 0]]
        self.origin, self.size, self.max = self._make_map(zx, self.padding, self.resolution)
    ...
    def _sample_points(self, env, N):
        ...
        for i in range(N):
            pts[i, :] = env.simulator.scene.get_random_point(self.floor_num)[1][[1, 2, 0]]
        return pts
