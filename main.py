...
def main():
    ...
    g_agent = algo.PPO(g_policy, args.clip_param, args.ppo_epoch, args.num_mini_batch,
                       args.max_batch_size, args.rotation_augmentation, args.value_loss_coef, args.action_loss_coef, args.predict_loss_coef,
    ...
    g_value, mi_loss , g_action, g_action_log_prob, g_rec_states, g_action_map = \
        g_policy.act(
    ...
    if args.centralized:
        ...
        for i in range(num_scenes):
            ...
            cluster_num = global_input[i, 1, :, :]
            for a in range(num_robots):
                g_history[(i, *frontier_idx[cpu_actions[i, a]])] = cluster_num[(*frontier_idx[cpu_actions[i, a]],)]
    ...
    for idx_episode in range(args.restore_eps, num_episodes):
        ...
        for step in range(args.max_episode_length):
            ...
            if l_step == args.num_local_steps - 1:
                ...
                g_rollouts.insert(
                    ...
                    g_rec_states, g_action, g_action_log_prob, g_value,mi_loss, g_reward_tensor,
            ...
            if (g_step % args.num_global_steps == args.num_global_steps - 1 and l_step == args.num_local_steps - 1) and g_rollouts.mini_step == 0:
                if args.train_global and not eval_flag:
                    ...
                    g_rollouts.compute_returns(g_next_value, args.use_gae,
                                               args.gamma, args.tau)
                    g_value_loss,mi_loss_loss, g_action_loss, g_dist_entropy, g_predict_loss = \
                        g_agent.update(g_rollouts)
                    if g_value_loss > 0:
                        g_value_losses.append(g_value_loss)
                        mi_loss_losses.append(mi_loss_loss)
                        g_action_losses.append(g_action_loss)
                        g_dist_entropies.append(g_dist_entropy)
                        g_predict_losses.append(g_predict_loss)
            ...
            if total_num_steps % args.log_interval == 0:
                ...
                if args.train_global and len(g_value_losses) > 0:
                    log += " ".join([
                        " Global Loss value/mi_loss/action/dist/predict:",
                        "{:.5f}/{:.5f}/{:.5f}/{:.5f}/{:.5f},".format(
                            np.mean(g_value_losses),
                            np.mean(mi_loss_losses),
                            np.mean(g_action_losses),
                            np.mean(g_dist_entropies),
                            np.mean(g_predict_losses))
                    ])
        ...
        if args.train_global and len(g_value_losses) > 0:
            ...
            summary_writer.add_scalar('loss/mi', mi_loss_losses[-1], global_step=idx_episode)
            summary_writer.add_scalar('loss/entropy', g_dist_entropies[-1], global_step=idx_episode)
            summary_writer.add_scalar('loss/predict', g_predict_losses[-1], global_step=idx_episode)
