...
class MapManager:
    ...
    def update_local(self, sensor_pose):
        ...
        for e in range(self.num_scenes * self.num_robots):
            ...
            agent_location_r, agent_location_c = agent_location[e, 1], agent_location[e, 0]
            agent_location_r = max(0, min(self.global_map_w-1, agent_location_r))
            agent_location_c = max(0, min(self.global_map_h-1, agent_location_c))
            self.global_map[e // self.num_robots, 3, agent_location_r, agent_location_c] = 1.
    ...
    def get_global_input(self, g_history):
        if self.centralized:
            ...
            global_input[:,1,:,:] = torch.from_numpy(self._generate_cluster_frontier(global_input[:,1,:,:])).to(self.device)
            ...
            for e in range(self.num_scenes):
                # make sure number of frontiers is bigger than robots   
                if torch.sum(global_input[e, 1, :, :] > 0) == 0:
                    global_input[e, 1, :, :][global_input[e, 2, :, :] > 0] = 1
    ...
    def _generate_cluster_frontier(self, frontier_map):
        # return map
        frontier_map_np = frontier_map.cpu().numpy()
        cluster_frontier_map = np.zeros_like(frontier_map_np)
        for e in range(frontier_map_np.shape[0]):
            frontiers_index_tuple = np.where(frontier_map_np[e,:,:]==1)
            # all detected frontiers
            frontier_index = np.stack(frontiers_index_tuple,axis=-1).tolist()
            # initialize a frontier cluster list as Clusters
            clusters = []

            while len(frontier_index)!=0:
                # initialize a cluster as NewCluster
                new_cluster = []
                # copy the first one of Froniters into NewCluster and remove it from Frontiers
                new_cluster.append(frontier_index.pop(0))
                # repeat
                temp_frontier = copy.copy(frontier_index)
                
                for item in frontier_index:
                    # if item is a neighbor of any frontier in NewCluster then copy item into NewCluster and delete item in Frontiers
                    for cluster_item in new_cluster:
                        # if item is a neighbor of any frontier in NewCluster
                        if abs(item[0]-cluster_item[0])<=1 and abs(item[1]-cluster_item[1])<=1:
                            new_cluster.append(item)
                            temp_frontier.remove(item)
                            break
                frontier_index = copy.copy(temp_frontier)
                clusters.append(new_cluster)
                # pick a random point as the center of cluster
                cluster_center_id = random.randint(0,len(new_cluster)-1)
                cluster_frontier_map[e][new_cluster[cluster_center_id][0]][new_cluster[cluster_center_id][1]]=len(new_cluster)
            
            # if nums of cluster is smaller than nums of robot but not because nums of unclustered frontier is smaller
            if len(clusters) < self.num_robots and clusters and any(sub_list for sub_list in clusters if clusters):
                for _ in range(self.num_robots-len(new_cluster)):
                    frontier_idx = np.nonzero(frontier_map_np[e,:,:])
                    num_frontier = len(frontier_idx[0])
                    pick_idx = random.randint(0,num_frontier-1)   
                    cluster_frontier_map[e][frontier_idx[0][pick_idx]][frontier_idx[1][pick_idx]]=len(new_cluster)             

        return cluster_frontier_map     
